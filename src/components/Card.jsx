...
const Card = ({ product }) => {
  ...
  const dispatch = useDispatch(); const [quantity, setQuantity] = useState(1);
  ...
  return (
    ...
    >
      ...
      <div className='flex flex-col items-center max-w-[250px] max-h-[300px]'>
        <Link to={`/product/${product.id}`} className="card-link"><img src={product.image} alt={product.name} className="card__image" /></Link>
        <div className='h-[35px] w-[125px] flex justify-center'>
          {isHovered && (
            <div className="card__icons flex items-center gap-x-6">
              <button onClick={handleAddToCart} className='addtoCart text-[#3D3D3D] flex justify-center items-center text-[22px] h-[35px] bg-white w-[35px] hover:text-[#46A358] duration-200'>
                <SlBasket/>
              </button>
